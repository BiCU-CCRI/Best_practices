...
- Do your code development in branches
    - Branches can be easily merged, reverted, commented on (Pull Request), and removed without affecting the `main` branch
...
- They serve as the primary project documentation for code development, together with the README (and the Issues description)
...
- An item without an assigned repository is a **Draft** ticket
    ...
    - This helps, for example, to distinguish Items with an unassigned repository by accident from those that really are drafts
- Assigning a repository to the Item *copies* the Item to the tagged repository and creates an **Issue**
...
3. The third option is usually used if we collaborate on a project with another person, need to fix something in the `main` branch fast, or the finished Subtask makes it easier for the other person to continue the development.
...
- Some ticket types might not need a full description if they are *simple* enough, so the commit/PR message can describe them well enough
...
- When writing the Issue description, you can use check-box markdown notation `- [ ]` in the description *body* and then click on `Convert to sub-issue` on the right
    - This creates a new Issue and moves the `- [ ]` *text* below the main description text box
    - **Note:** You can also use the same feature to create new Issues, not just Sub-issues
...
- Branches can be reviewed, merged, deleted, and reverted
...
- Regex: `^\d+-(feature|subtask|experiment|test|janitor|bugfix|hotfix|release)-([a-zA-Z0-9]+(-[a-zA-Z0-9]+)*)$`
    ...
    - **Note:** The name structure follows GitHub's *automatic*
...
- The next word of the commit message starts with a **capital letter**
    - This makes it clear that the commit message is complete and as intended
...
- **Improve** - For optimization or quality improvements.
    - Example: `Improve the performance of the sorting algorithm`
...
- **Reorganize** - For changing the project's *higher* structure.
...
- **Revert** - For undoing previous commits.
    - Example: `Revert changes to the login feature`
...
- An exception to this rule is when the main implemented feature(s) significantly differ from the original ticket name and content
    - You can also consider changing the Issue title, but be cautious not to break any links and collaborators' local repository
...
- **Note:** Unfortunately, GitHub doesn't allow setting rulesets separately for PR merge and *regular* commit messages (PR merge is technically just another commit). We cannot include the `(feature|subtask|experiment|test|janitor|bugfix|hotfix|release)` in the rulesets
...
- Do not confuse the **PR commit message** with **PR comment** (=PR comment is **not** included in the `git` history)
    - Unfortunately, GitHub puts PR comments on top, making it slightly less obvious what is what
    ...
    - **Note:** You can use the `- [ ]` markdown check-box option to keep track of your own development progress on both the Subtask and Feature levels
...
- PR is initiated by the developer:
...
11. Merge the PR once all the comments/suggestions are addressed, resolved (by the Reviewer), and the PR is approved by the Reviewer
...
15. Approve the PR once you are happy with the status of the code

- By submitting comments/suggestions this way, all your comments/suggestions will be marked as *Pending* and submitted only once at the very end of this PR round.
...
- GitHub offers a lot of [rules/rulesets](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/about-rulesets) to help us enforce the agreed-upon *rules* for commit names, branch names, PR rules, branch protection rules, etc.
...
- Some *sections* are ok with having `#` as the title/message character (`git commit` message, PR title), while some struggle (switching branches in the command line)
- The current title/message structure comes from the *least necessary* changes
- In the optimal case, the Issue IDs would start with a custom ID that can be used in all three names (=starting with anything but `#`)
...
- Both provide a **DOI to your code** - *freeze* the software code, versio, and allow citations
...
1. Create a tag on GitHub. For example, `v1.0`. You could also use something like `v1.0_prepub`
2. Log in to Zenodo.com with your GitHub account. You can also use ORCID or just create a user account. I find GitHub the easiest since I only upload code.
3. Upload the tagged GitHub code to Zenodo.
4. Add description if not included by default. For example, **Is a supplement to** and add the link to your GitHub repo.
5. Finish

- If you get any revision requests or you want to update something, you can update your *Zenodo* repo with the latest code
- Zenodo then creates two DOIs - one for the *repo* and one for each uploaded *version*. You can then decide if you want to use the *repo* DOI or the specific version in your paper. You can upload multiple GitHub tags (versions), and each of the *versions* will get its own DOI. The main *repo* DOI stays the same regardless of the version of the code
