...
- Standardizing the project (~repository) structure and code organization further improves code development clarity
...
- **`.gitignore`** - `.gitignore` file
    - The following directories/files are commonly included: `bin`, `data`, `logs`, `notebooks`, `publication`, `reports`, `results`
- **`bin`** - additional binaries that are not installed through `conda` or virtual environments or are not run through `Docker`/`Apptainer`. Instructions on installing additional software to `bin` should be described in `run/run_build_envs.sh` or `envs`
- **`config`** - configuration files for the project and analyses
- **`data`** - raw data, databases, references, and other necessary input data. Prefer linking from shared directories whenever possible to avoid unnecessary duplication of large files
    - **`data/raw`** -  raw input data for the project; this data does not change
...
- **`docs`** - additional documentation for the project, such as:
    ...
    - **`docs/publication`** copies of relevant PDF materials
...
- **`src`** - scripts for individual analysis steps. Workflow-specific scripts can also be placed within the `workflow` directory(s) if the project is based on workflows.
    - **`src/envs`** - additional scripts/instructions on how to build environments
    - **`src/bash`** - bash scripts
    - **`src/python`** - Python scripts
    - **`src/R`** - R scripts
- **`workflow`** - workflow for the project. Follow the best practices specified for the workflow manager. The workflow *type* (`snakemake`/`nextflow`) should be reflected in the repo name. If not, we can create an additional subdirectory structure `workflow/snakemake`/`workflow/nextflow`.
...
**Note:** For research-heavy projects where we don't have an *exact flow* of the code, it's a good idea to prefix your script names with `01_`, `02_`, `03_` to highlight the order of scripts in the analysis/exploration.
...
- `nextflow` recommends using `nf-core pipelines create` to set up a new pipeline [repository structure](https://nf-co.re/docs/guidelines/pipelines/requirements/use_the_template)
...
- Please, write down from where you downloaded the image or how you built it in the respective shared directories in `run.sh`
    - This file can be `git` tracked and reused, redownload the image if necessary, or used for the manuscript's methods section
