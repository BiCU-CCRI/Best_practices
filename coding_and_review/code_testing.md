...
- **Note:**  every user's behavior
    - We can limit certain behaviors in the README of the function input limits
    - For example, we can specify that the input to the function is a `fastq` file - we do not have to test whether the input is a non-empty `fastq` file, as it is not an issue of the code but of the user themselves
...
- **Note:** As E2E tests usually require bigger test files, be careful when uploading them to GitHub
    - The total GitHub repository should ideally be [smaller than 1 GB](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github) (including the history)
...
- Manually setting up tests for each function might take some time
    - One has to think about possible scenarios and write them in the test function
    - Luckily, most code editors have add-ins that help create unit tests, so you don't have to build everything from scratch!
        - **Note:** The add-ins are usually good in providing a unit test *backbone* - you still have to check the suggestion and adjust it according to the *real* theoretical inputs/outputs
...
- Optimally, we would run all our analyses in containers that are very easy to use for testing
    - Unfortunately, neither CCRI nor CeMM clusters have full container support
...
- **Note:** GitLab has better CI/CD integration than GitHub because it is self-hosted and can run the tests locally. GitHub runs the tests in online virtual machines.
